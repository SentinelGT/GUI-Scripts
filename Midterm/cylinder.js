...
function CylinderBase( slices, stacks, vertexShader, fragmentShader )
{ 
    var i, j;
	
    var program = initShaders(gl, vertexShader || "Cyl-vertex-shader", fragmentShader || "Cyl-fragment-shader");
	...
	// uses cylindrical coordinates instead of circular coordinates
    //I did it backwards or something?
    for (j = 0; j < nStacks; ++j)
	{
        var z = 1.0 - (j * dZ);
	...
	//Firt top portion of the first cylinder
    indices.push(0);
	
	//Just load the indices in sequentially.
    for (i = 0; i < nSlices; ++i)
	{
        // loop through for each cylinder for the 'spokes' specificaly
        indices.push(n + i);
    }
